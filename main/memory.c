...
esp_err_t memory_set(const char *key, const char *value)
{
    ...
    if (key_len == 0 || key_len > MEMORY_NVS_KEY_MAX_LEN) {
        ESP_LOGE(TAG, "Refusing to set '%s': key length %zu exceeds NVS limit %d", key, key_len, MEMORY_NVS_KEY_MAX_LEN);
...
{
    if (!key || !value || max_len == 0) return false;
    size_t key_len = strlen(key);
    if (key_len == 0 || key_len > MEMORY_NVS_KEY_MAX_LEN) {
        ESP_LOGW(TAG, "Refusing to get '%s': key length %zu exceeds NVS limit %d", key, key_len, MEMORY_NVS_KEY_MAX_LEN);
        return false;
    }
...
esp_err_t memory_delete(const char *key)
{
    ...
    size_t key_len = strlen(key);
    if (key_len == 0 || key_len > MEMORY_NVS_KEY_MAX_LEN) {
        ESP_LOGW(TAG, "Refusing to delete '%s': key length %zu exceeds NVS limit %d", key, key_len, MEMORY_NVS_KEY_MAX_LEN);
        return ESP_ERR_NVS_KEY_TOO_LONG;
    }
